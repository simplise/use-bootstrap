...
definePageMeta({
  'title:en': 'Spacing',
  'description:en': `Bootstrap includes a wide range of shorthand responsive margin, padding, and gap utility classes to modify an element's appearance.`,
  'title:ja': 'スペーシング',
  'description:ja': '要素の外観を変更するための margin, padding, gap utility が含まれています。'
})
</script>
...
Assign responsive-friendly `margin` or `padding` values to an element or a subset of its sides with shorthand classes. Includes support for individual properties, all properties, and vertical and horizontal properties. Classes are built from a default Sass map ranging from `.25rem` to `3rem`.
 ...
 -->
  ...
  <Localization>
    <template #en>
      <p>Using the CSS Grid layout module? Consider using [the gap utility](#gap).</p>
    </template>
    <template #ja>
      <p>CSS Grid layout moduleを使用している場合は the gap utility の使用を検討してください。</p>
    </template>
  </Localization>
  ...
  <Localization>
    <template #en>
      <p>Spacing utilities that apply to all breakpoints, from <code>xs</code> to <code>xxl</code>, have no breakpoint abbreviation in them. </p>
      <p>This is because those classes are applied from <code>min-width: 0</code> and up, and thus are not bound by a media query. The remaining breakpoints, however, do include a breakpoint abbreviation.</p>
    ...
    <template #ja>
      <p><code>xs</code> から <code>xxl</code> までのすべてのブレークポイントに適用されるスペーシングユーティリティには、ブレークポイントの略語が含まれていません。 </p>
      <p>これらのクラスは <code>min-width: 0</code> 以降に適用されるため、メディアクエリに縛られないからです。しかし、残りのブレークポイントにはブレークポイントの略語が含まれています。</p>
      <p>クラスは，<code>xs</code> の場合は <code>{property}={sides}-{size}</code> という形式で <code>sm</code>、<code>md</code>、<code>lg</code>、<code>xl</code>、<code>xxl</code> の場合は <code>{property}={sides}-{breakpoint}-{size}</code> という形式になります。</p>
    </template>
  </Localization>
  ...
  <ul>
    ...
    <li> <code>padding</code> - for classes that set <code>padding</code></li>
  </ul>
...
-->
  ...
  <Localization>
    <template #en>
      <p>Additionally, Bootstrap also includes an <code>margin="x-auto"</code> attribute for horizontally centering fixed-width block level content—that is, content that has <code>display: block</code> and a <code>width</code> set—by setting the horizontal margins to <code>auto</code>.</p>
    </template>
    <template #ja>
      <p>Bootstrapには、ブロックレベルのコンテンツ、つまり <code>display: block</code> と <code>width</code> が設定されているコンテンツを、水平方向の余白を <code>auto</code> に設定することで水平方向にセンタリングするための <code>margin="x-auto"</code> 属性を用意しています。</p>
    </template>
  </Localization>

  <Snippet>
    <Example flex>
      <examples-utilities-spacing-horizontal-centering />
    </Example>
    <ExamplesCode
      path="/utilities/spacing/horizontal-centering.vue"
      lang="vue"
    />
  </Snippet>
...
``` -->
  ...
  <Localization>
    ...
    <template #ja>
      <p><code>display：grid</code>を使用する場合、親 Grid コンテナで <code>gap</code> utilities を利用できます。 これにより、個々のグリッドアイテム（ display：grid コンテナの子）に margin utilities を追加する手間を省くことができます。 gap utilities はデフォルトでレスポンシブであり, <code>$spacers</code> Sassマップに基づいて utilities API を介して生成されます。</p>
    </template>
  </Localization>

  <Snippet>
    <Example cssgrid>
      <examples-utilities-spacing-gap />
    </Example>
    <ExamplesCode
      path="/utilities/spacing/gap.vue"
      lang="vue"
    />
  </Snippet>
  ...
  <Localization>
    <template #en>
      <p>
        Support includes responsive options for all of Bootstrap's grid breakpoints, as well as six sizes from the <code>$spacers</code> map (<code>0</code>–<code>5</code>). There is no <code>gap=auto</code> utility class as it's effectively the same as <code>gap=0</code>.
      </p>
    </template>
    <template #ja>
      <p>
        サポートには、Bootstrapのすべての Grid ブレークポイント に対するレスポンシブオプションと, <code>$spacers</code> マップからの6つのサイズ(<code>0</code>–<code>5</code>）が含まれます。 <code>gap=auto</code> utility クラスは実質的に <code>gap=0</code>と同じであるため、存在しません。
      </p>
    </template>
  </Localization>
  <!-- ## Sass
