import { defineNuxtModule, createResolver, installModule, addPlugin, addComponentsDir, addComponent, addImportsDir, addServerHandler } from '@nuxt/kit';
...
const module = defineNuxtModule({
  ...
  defaults: {
    ...
    serverStorage: {
      api: {},
      path: "/api/storage",
      password: "83F4AE56-0370-4AB6-A754-D047F2A11AD1"
    }
  },
  async setup(options, nuxt) {
    const resolver = createResolver(import.meta.url);
    nuxt.options.runtimeConfig.public.usebootstrap = defu(nuxt.options.runtimeConfig.public.usebootstrap, options);
    ...
    addComponentsDir({ path: resolver.resolve("./runtime/components/extend"), prefix: options.extend?.prefix, pathPrefix: false });
    addComponent({ name: "SwiperSlide", export: "SwiperSlide", filePath: "swiper/vue" });
    addImportsDir(resolver.resolve("runtime/composables/public"));
    const isProdEnv = process.env.NODE_ENV === "production";
    if (!isProdEnv && fs.existsSync(resolver.resolve("runtime/components/dev/"))) {
      addComponentsDir({ path: resolver.resolve("runtime/components/dev/"), prefix: options.extend?.prefix, pathPrefix: false });
    }
    addComponent({ name: `${options.combadge?.prefix}Data`, filePath: resolver.resolve("./runtime/components/combadge/c-data") });
    addComponent({ name: `${options.combadge?.prefix}ListItem`, filePath: resolver.resolve("./runtime/components/combadge/c-list-item") });
    addComponent({ name: `${options.combadge?.prefix}Label`, filePath: resolver.resolve("./runtime/components/combadge/c-label") });
    addComponent({ name: `${options.combadge?.prefix}Errors`, filePath: resolver.resolve("./runtime/components/combadge/c-errors") });
    addComponent({ name: `${options.combadge?.prefix}Input`, filePath: resolver.resolve("./runtime/components/combadge/c-input") });
    addComponent({ name: `${options.combadge?.prefix}Check`, filePath: resolver.resolve("./runtime/components/combadge/c-input-check") });
    addComponent({ name: `${options.combadge?.prefix}Button`, filePath: resolver.resolve("./runtime/components/combadge/c-button") });
    addComponent({ name: `${options.combadge?.prefix}Link`, filePath: resolver.resolve("./runtime/components/combadge/c-link") });
    addComponent({ name: `${options.combadge?.prefix}Li`, filePath: resolver.resolve("./runtime/components/combadge/c-list-item") });
    addComponent({ name: `${options.combadge?.prefix}Tr`, filePath: resolver.resolve("./runtime/components/combadge/c-table-row") });
    addComponent({ name: `d-data`, filePath: resolver.resolve("./runtime/components/combadge/vd-data") });
    addPlugin(resolver.resolve("./runtime/plugins/vueuse"));
    addComponentsDir({ path: resolver.resolve("./runtime/components/vueuse"), prefix: options.extend?.prefix, pathPrefix: false });
    const alphabets = [..."abcdefghijklmnopqrstuvwxyz"];
    let route = options.serverStorage.path;
    alphabets.forEach((n) => {
      route = `${route}/:${n}`;
      addServerHandler({
        route,
        handler: resolver.resolve("./runtime/server/api/storage/[...slug]")
      });
    });
  }
});
