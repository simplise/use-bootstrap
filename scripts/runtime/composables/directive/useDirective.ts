import { spacing, toArray, hasValue } from '../utils/useProps';

export function useDirective(element: HTMLElement | undefined) {
  ...
  for (const attr of element.attributes) {
   switch (attr.name) {
    ...
    case 'opacity':
     ...
     break;
    case 'placeholder': // wrong?
     element.classList.add(
      `placeholder${hasValue(attr.value) ? `-${attr.value}` : ''}`,
     );
     break;
    case 'placeholder-size': // wrong?
...
function addClassNames<T>(value: T, func: ClassNameGenerator) {
